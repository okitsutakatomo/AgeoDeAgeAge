...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
	...
	this.loadAccessToken = function(pService) {
		...
		Ti.API.debug('Loading access token: done [accessToken:' + accessToken + '][accessTokenSecret:' + accessTokenSecret + '][userId:' + userId + '][screenName:' + screenName + '].');
	};
	...
	this.showAuthorizeUI = function(pUrl, pReceivePinCallback) {
		...
		window.rightNavButton = closeButton;
	...
	this.getAccessToken = function(pUrl) {
		...
		userId = responseParams['user_id'];
		screenName = responseParams['screen_name'];
